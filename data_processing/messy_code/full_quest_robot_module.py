...
class QuestLeftArmGripperModule(QuestRobotModule):
    ...
    def solve_fingertip_ik(self, fingertip_pos):
        ...
        # NOTE: Algin the gripper with human hand!
        midpoint_pos = middle_pos  # debug --> it should aligned with middle finger!!
        # midpoint_pos = (8*thumb_pos + index_pos + 7*middle_pos)/16  # 8:1:7
        # midpoint_pos = (2*thumb_pos + index_pos + index_pos)/4    # Origional
...
class QuestLeftArmGripperNoRokokoModule(QuestRobotModule):
    ...
    left_palm_pos_orn_offset = np.array([0.03, 0.0, 0.04, 0.0, 0.0, -np.pi/2-np.pi/8])
