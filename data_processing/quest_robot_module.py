...
class QuestLeftArmGripperNoRokokoModule(QuestRobotModule):
    # Virtual robot arm and hand pose
    ARM_REST = [0.,
                -0.49826458111314524,
                -0.01990020486871322,
                -2.4732269941140346,
    ...
    LEFT_HAND_Q = [-0.04, -0.04]    # 1DOF parallel jaw gripper
    ...
    # TODO: Find the fit offset 
    ## XYZ on the left wrist --> Z points down, X points forward, Y points right (after rotation for [-np.pi/2-np.pi/8])
    # left_palm_pos_orn_offset = np.array([0.1, 0.03, 0.04, 0.0, 0.0, -np.pi/2-np.pi/8])    # Eric
    left_palm_pos_orn_offset = np.array([0.12, 0.0, 0.05, 0.0, 0.0, -np.pi/2-np.pi/8])  # Zi-ang
    # left_palm_pos_orn_offset = np.array([0.12, 0.0, 0.05, 0.0, 0.0, -np.pi/2-np.pi/8])  # Zi-ang
    ...
    def receive(self):
        ...
        if data_string.startswith("WorldFrame"):
            ...
            # Check whether the hand is open or close
            if (data_list[14] > 0.08):
                self.is_open = True
            else:
                self.is_open = False
